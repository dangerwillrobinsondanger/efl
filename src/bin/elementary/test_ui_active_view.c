...
{
   Eo *pager = data;
   int curr_page = efl_ui_active_view_active_index_get(pager);

   if (curr_page - 1 > -1)
   efl_ui_active_view_active_index_set(pager, curr_page -1);
}
...
static void next_btn_cb(void *data, const Efl_Event *ev EINA_UNUSED)
{
   ...
   if (curr_page + 1 < efl_content_count(pager))
     efl_ui_active_view_active_index_set(pager, curr_page +1);
}
...
static void width_slider_cb(void *data, const Efl_Event *ev)
{
   ...
   efl_ui_active_view_size_set(params->pager, EINA_SIZE2D(params->w, h));
}
...
static void height_slider_cb(void *data, const Efl_Event *ev)
{
   ...
   efl_ui_active_view_size_set(params->pager, EINA_SIZE2D(w, params->h));
}
...
static void width_check_cb(void *data, const Efl_Event *ev)
{
   ...
   efl_ui_active_view_size_set(params->pager, EINA_SIZE2D(w, h));
}
...
static void pack_btn_cb(void *data, const Efl_Event *ev EINA_UNUSED)
{
   ...
   switch (param->type) {
      ...
      case PACK_END:
         ...
         break;
      case PACK_BEFORE:
         index = efl_ui_active_view_active_index_get(pager);
...
static void page_set_btn_cb(void *data, const Efl_Event *ev EINA_UNUSED)
{
   ...
   efl_ui_active_view_active_index_set(psp->pager,
                                 efl_ui_range_value_get(psp->spinner));
}

static void page_set_del_btn_cb(void *data, const Efl_Event *ev EINA_UNUSED)
{
   Page_Set_Params *psp = data;

   efl_ui_active_view_active_index_deletion(psp->pager,
                                 efl_ui_range_value_get(psp->spinner));
}
...
static void indicator_icon_btn_cb(void *data, const Efl_Event *ev EINA_UNUSED)
{
   ...
   params->indicator = efl_add(EFL_UI_ACTIVE_VIEW_INDICATOR_ICON_CLASS, params->pager);
   efl_ui_active_view_indicator_set(params->pager, params->indicator);
}

static void indicator_none_btn_cb(void *data, const Efl_Event *ev EINA_UNUSED)
{
   Params *params = data;
   efl_ui_active_view_indicator_set(params->pager, NULL);
}
...
_gravity_changed_cb(void *data, const Efl_Event *ev)
{
   ...
   if (efl_ui_nstate_value_get(ev->object) == 0)
     {
        efl_ui_active_view_gravity_set(params->pager, EFL_UI_ACTIVE_VIEW_CONTAINER_GRAVITY_INDEX);
     }
   else
     {
        efl_ui_active_view_gravity_set(params->pager, EFL_UI_ACTIVE_VIEW_CONTAINER_GRAVITY_CONTENT);
     }
}

static void view_index_gravity_cb(void *data,
                         Evas_Object *obj EINA_UNUSED,
                         void *event_info EINA_UNUSED)
{
   Params *params = data;
   ...
   efl_pack_end(box, ck);
   efl_gfx_entity_visible_set(ck, 1);
   if (efl_ui_active_view_gravity_get(params->pager) == EFL_UI_ACTIVE_VIEW_CONTAINER_GRAVITY_CONTENT)
     {
...
_animation_cb(void *data, const Efl_Event *ev)
{
   ...
   efl_ui_active_view_view_manager_animation_set(efl_ui_active_view_manager_get(params->pager), efl_ui_nstate_value_get(ev->object));
}

static void view_animation_cb(void *data,
                         Evas_Object *obj EINA_UNUSED,
                         void *event_info EINA_UNUSED)
{
   Params *params = data;
   ...
   efl_ui_nstate_value_set(ck, efl_ui_active_view_view_manager_animation_get(efl_ui_active_view_manager_get(params->pager)));
                    ...
                    void *event_info EINA_UNUSED)
{
   ...
   sp1 = efl_add(EFL_UI_SPIN_BUTTON_CLASS, in_box1,
                 ...
                 efl_ui_range_value_set(efl_added,
                                        efl_ui_active_view_active_index_get(pager)));
                            ...
                            void *event_info EINA_UNUSED)
{
   ...
   btn2 = efl_add(EFL_UI_BUTTON_CLASS, box,
                 efl_text_set(efl_added, "Remove Active Index & Set"),
                 efl_pack_end(box, efl_added));
   ...
   efl_event_callback_add(btn2, EFL_UI_EVENT_CLICKED, page_set_del_btn_cb, psp);
}
                                   ...
                                   void *event_info EINA_UNUSED)
{
  Eo *win, *panes, *navi, *list, *layout, *pager, *page;
   ...
   pager = efl_add(EFL_UI_ACTIVE_VIEW_CONTAINER_CLASS, layout,
                   efl_content_set(efl_part(layout, "pager"), efl_added),
                   efl_ui_active_view_size_set(efl_added, EINA_SIZE2D(200, 300)));

   efl_ui_active_view_manager_set(pager, efl_new(EFL_UI_ACTIVE_VIEW_VIEW_MANAGER_STACK_CLASS));
   ...
   elm_list_item_append(list, "View Size", NULL, NULL, page_size_cb, params);
   elm_list_item_append(list, "Pack / Unpack", NULL, NULL, pack_cb, params);
   elm_list_item_append(list, "Active Index", NULL, NULL, current_page_cb, params);
                                   ...
                                   void *event_info EINA_UNUSED)
{
  Eo *win, *panes, *navi, *list, *layout, *pager, *page;
   ...
   efl_ui_active_view_manager_set(pager, efl_new(EFL_UI_ACTIVE_VIEW_VIEW_MANAGER_SCROLL_CLASS));

...
{
   Eo *stack = data;
   efl_ui_active_view_active_index_deletion(stack, 0);
}
...
_fifth_layout_insert(void *data, const Efl_Event *ev EINA_UNUSED)
{
   ...
   efl_pack_begin(stack, nl);
   efl_ui_active_view_active_index_set(stack, 0);
}
...
test_ui_stack(void *data EINA_UNUSED, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
   ...
   Eo *stack = efl_add(EFL_UI_ACTIVE_VIEW_CONTAINER_CLASS, win);
   efl_ui_active_view_manager_set(stack, efl_add(EFL_UI_ACTIVE_VIEW_VIEW_MANAGER_STACK_CLASS, stack));

...
typedef struct _Efl_Ui_Active_View_Container_Data
{
   ...
   struct {
     Efl_Gfx_Entity *removal;
   } post_transition_removal;
...
_transition_end(Eo *obj EINA_UNUSED, Efl_Ui_Active_View_Container_Data *pd)
{
   ...
   if (pd->post_transition_removal.removal)
     {
        efl_del(pd->post_transition_removal.removal);
        pd->post_transition_removal.removal = NULL;
     }
...
_efl_ui_active_view_container_efl_object_finalize(Eo *obj, Efl_Ui_Active_View_Container_Data *pd EINA_UNUSED)
{
   ...
   efl_ui_active_view_manager_set(obj, efl_add(EFL_UI_ACTIVE_VIEW_VIEW_MANAGER_PLAIN_CLASS, obj));
...
_update_internals(Eo *obj EINA_UNUSED, Efl_Ui_Active_View_Container_Data *pd, Efl_Gfx_Entity *subobj EINA_UNUSED, int index)
{
   ...
   if (pd->transition)
     efl_ui_active_view_view_manager_content_update(pd->transition, subobj, index, EINA_FALSE);
   if (pd->indicator)
     efl_ui_active_view_indicator_content_update(pd->indicator, subobj, index, EINA_FALSE);
                               ...
                               int index)
{
   ...
   pd->curr.page = index;
   efl_ui_active_view_view_manager_request_switch(pd->transition, old_curr_page, pd->curr.page);
}
        ...
        int index)
{
   ...
   if (pd->transition)
     efl_ui_active_view_view_manager_content_update(pd->transition, subobj, index, EINA_TRUE);
   if (pd->indicator)
     efl_ui_active_view_indicator_content_update(pd->indicator, subobj, index, EINA_TRUE);
...
_efl_ui_active_view_container_view_manager_set(Eo *obj, Efl_Ui_Active_View_Container_Data *pd, Efl_Ui_Active_View_View_Manager *transition)
{
   ...
   if (pd->transition)
     ...
     {
        //in case that a view manager uses a clipper, unset them here
        EINA_LIST_FOREACH(pd->content_list, n, content)
          {
             efl_canvas_object_clipper_set(content, NULL);
          }
     }
   ...
   if (pd->transition)
     {
        ...
        efl_ui_active_view_view_manager_animation_set(pd->transition, EINA_TRUE);
...
EOLIAN static void
_efl_ui_active_view_container_active_index_deletion(Eo *obj, Efl_Ui_Active_View_Container_Data *pd, int new_index)
{
   //this will be removed when a transition end is emitted
   pd->post_transition_removal.removal = eina_list_nth(pd->content_list, pd->curr.page);
   //trigger a animation / request change, in order to get a new transition and a removal at some point

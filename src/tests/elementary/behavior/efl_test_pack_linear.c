...
EFL_START_TEST(pack_at3)
{
   ...
   ck_assert_ptr_eq(efl_ui_widget_parent_get(inv), widget);
   ck_assert_int_eq(efl_pack_at(widget, inv2,-100000), EINA_FALSE);
...
EFL_START_TEST(pack_content_get2)
{
   Efl_Ui_Widget *wid[3];
   ...
   for (int i = -1000; i < -3; ++i)
     {
        printf("%d\n", i);
        ck_assert_ptr_eq(efl_pack_content_get(widget, i), NULL);
     }
...
efl_pack_linear_behavior_test(TCase *tc)
{
   ...
   tcase_add_test(tc, pack_content_get2);
